...
// 2. Remove Keamanan
// from `<div className="h-px bg-input mx-4" />\n\n            {/* Keamanan */}`
// up to `          </div>\n        </div>\n\n        {/* ── Tentang ─────────────────────────────────────── */}`
code = code.replace(
...
// Add `const [notifStatus, setNotifStatus] = useState<string>('default');`
code = code.replace(
...
// Add useEffect check permission
code = code.replace(
  /  useEffect\(\(\) => \{\n    let mounted = true;/,
  `  useEffect(() => {
    let mounted = true;
...
const tentangOld = `<div className="flex flex-col items-center justify-center p-6 text-center gap-4">
...
code = code.replace(tentangOld, tentangNew);
